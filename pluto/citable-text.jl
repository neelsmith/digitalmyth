...
"""Read a two-column source file from a URL, and return a pair of vectors.
Also be tidy and remove temporary downloaded file.
"""
function readurl(u; separator = "|")
	f = Downloads.download(u)
	data = readlines(f)
	rm(f)
	columns = map(row -> split(row, separator), data)
	reff = map(cols -> cols[1], columns)
	text = map(cols -> cols[2], columns)
	(reff, text)
end

# ╔═╡ d1776932-992c-4d9a-a1ed-9180e7e3f7cf
citations, contents = readurl(apollodorus)
...
f = Downloads.download(apollodorus)

# ╔═╡ 15cbaba8-3b47-4a3c-848e-10b02496548f
data = readlines(f)

# ╔═╡ 9ae2f33b-ee90-4ad8-b18c-89c5941fd46b
columns = map(row -> split(row, "|"), data)

# ╔═╡ a082f77f-84d9-4118-9c2a-40533f8a32ed
reff = map(cols -> cols[1], columns)

# ╔═╡ 979a6ef8-acd8-48f7-b165-95f8c4d1468a
reff[2]

# ╔═╡ 42bf2c45-aed0-4562-9fc6-4c85f3a96e85
text = map(cols -> cols[2], columns)

# ╔═╡ aea4dfbd-5867-435f-b6bd-065d2f173a7e
text[2]
...
md"""*Search text for*:  $(@bind term TextField(; default = "Theseus"))"""

# ╔═╡ 30998652-a10e-4056-bff2-2d01bd7c7a0e
findall(paragraph -> contains(paragraph, term), text)

# ╔═╡ 378c7734-0b20-4afd-b3f6-1aa9d4d29aac
text[43] |> Markdown.parse

# ╔═╡ 88e4e7af-ffc4-4174-a00d-bf2edbec7e36
reff[43]

# ╔═╡ a0fd8678-f1d5-466b-b19e-95e5acfd7fef
md"""**$(reff[43])** *$(text[43])*"""
